...
function cardMarkup({store,url,type},mode,direct=false){
 ...
 const src=capture;
 const preview=src?`<img src="${escapeHtml(src)}" alt="${escapeHtml(store.name)} · ${pageLabel(type)} · ${mode==='mobile'?'Mobile':'Desktop'}" loading="lazy" />`:'<span class="capture-unavailable">Captura no disponible</span>';
...
function renderGallery(collection,state,target,categoryId,pageId,countId){
 ...
 document.getElementById(countId).textContent=`${entries.length} ${state===directoryState?(entries.length===1?'template':'templates'):(entries.length===1?'website':'websites')}`;
